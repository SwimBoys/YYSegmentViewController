...
open class YYSegmentConfig: NSObject {
    
    // MARK: - 显示的控制器
    /// 是否继承 YYSegmentViewController
    public var isInherit: Bool = false
    /// 显示的控制器数组
    public var containerControllerArr: [UIViewController] = []
    ...
    /// 分段控制器的高度，默认为50
    public var segmentControlHeight: CGFloat = 50
    /// 分段控制器的宽度，默认为屏幕宽
    public var segmentControlWidth: CGFloat = UIScreen.main.bounds.width
    /// 分段控制器的位置，默认在顶部
    public var segmentControlPositionType: YYSegmentedCtontrolPositionType = .top
    /// 是否显示竖向滚动指示器，默认隐藏
    ...
    public var isShowBottomSeparatorLineView = false
    /// 分段控制器底部分割线颜色，默认灰色
    public var bottomSeparatorLineViewBackgroundColor = UIColor.lightGray
    /// 分段控制器底部分割线线宽，默认 1
    public var bottomSeparatorLineViewHeight: CGFloat = 1 / UIScreen.main.scale
    /// 下拉刷新控件的位置，默认是列表下拉
    ...
    /// item默认选中角标，默认0
    public var itemDefaultSelectedIndex: NSInteger = 0
    /// item 底部ScrollView的 contentInset
    public var itemContentInset = UIEdgeInsets.zero
    /// item 宽度类型，默认是根据文本展示（左右各10）
    public var itemWidthStyle: YYItemViewWidthChangeStyle = .equalToTitleWidth(margin: 10)
    /// item 之间分割线是否显示，默认不显示
    public var isShowItemSeparatorLineView = false
    /// item 之间分割线颜色，默认 灰色
    public var itemSeparatorLineColor = UIColor.lightGray
    /// item 之间分割线宽度，默认 1
    public var itemSeparatorLineWidth: CGFloat = 1 / UIScreen.main.scale
    /// item 之间分割线距离上下的距离，默认都是 0
    public var itemSeparatorLineTopBottomMargin:(top: CGFloat, bottom: CGFloat) = (0, 0)
    /// item 文本默认颜色，默认 灰色
    public var itemTitleNormalColor: UIColor = .lightGray
    /// item 文本选中颜色，默认 黑色
    public var itemTitleSelectColor: UIColor = .black
    /// item 文本中心偏移距离
    public var itemTitleCenterOffsetY: CGFloat = 0
    /// item 文本font，默认 12
    public var itemTitleFont: CGFloat = 12
    /// item 选中过程过度大小
    public var itemTitleSelectedScale: CGFloat = 1.2
    /// item 选中的变化位置，默认是逐渐变化
    public var itemViewSegmentSelectedStyle: YYSegmentItemViewSelectedStyle = .gradient
    // MARK: - item 指示标
    /// item 指示标背景色，默认黑色
    public var itemIndicatorViewBackgroundColor: UIColor = .black
    /// item 指示标的位置（中间、偏上、偏下），默认是在底部
    public var itemIndicatorViewCenterYGradientStyle: YYIndicatorViewCenterYGradientStyle = .bottom(margin: 0)
    /// item 指示标宽度的类型，默认是固定宽度，为 10
    public var itemIndicatorViewWidthChangeStyle: YYIndicatorViewWidthChangeStyle = .stationary(baseWidth: 10)
    ...
    /// 角标是否显示
    public var isShowItemBadge = false
    /// 角标样式，默认是圆点
    public var itemBadgeStyle: YYItemViewBadgeStyle = .dots
    /// 角标文本颜色，默认白色
    public var itemBadgeTitleColor: UIColor = .white
    /// 角标背景颜色，默认红色
    public var itemBadgeBackgroundColor: UIColor = .red
    /// 角标字体大小，默认12
    public var itemBadgeTitleFont: CGFloat = 12
    /// 角标最大值，默认 99
    public var itemBadgeValueMaxNum: NSInteger = 99
    /// 角标style为dots和round时， 偏离item label 右上角，当style为 right 时，偏离文本的右边，y 不起作用
    public var itemBadgeValueLabelOffset = CGPoint.init(x: 0, y: 0)
    /// 角标style为圆点时， size默认（5，5）
    public var itemBadgeSize = CGSize(width: 5, height: 5)
    /// 角标style为round时， 角标文本距离上下左右的间距
    public var itemBadgeStyleRoundMargin: (leftRightMargin: CGFloat, topBottmMargin: CGFloat) = (4, 4)
}
